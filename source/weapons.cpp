	...
	ImGui::Begin("DrawWeapon", reinterpret_cast<bool*>(true), ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoInputs);
	{
		...
		if (CheckDraw() && g_pEngine->GetMaxClients())
		{
			...
			if (cvar.misc_weapon_local && g_Local.pEnt->curstate.weaponmodel && g_Local.bAlive && GetTickCount() - delay < 5000)
			{
				...
				ImGui::GetCurrentWindow()->DrawList->AddRect({ 20 - 2, (float)y + 52 }, { label_size.x + 24, (float)y + 66 }, black(), cvar.visual_rounding);
				ImGui::GetCurrentWindow()->DrawList->AddRect({ 20 - 3, (float)y + 51 }, { label_size.x + 23, (float)y + 65 }, wheel1(), cvar.visual_rounding);
				ImGui::GetCurrentWindow()->DrawList->AddText({ 20, (float)y + 50 }, white(), GetWeaponName(g_Local.pEnt->curstate.weaponmodel));
			}
		}
		ImGui::End();
		ImGui::PopStyleColor();
