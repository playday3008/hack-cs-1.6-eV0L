...
void V_CalcRefdefRecoil(struct ref_params_s *pparams)
{
	if (IsCurWeaponGun())
	{
		if (cvar.rage_active)
		{
			g_Local.vNoRecoilAngle[0] = pparams->punchangle[0] * cvar.rage[g_Local.weapon.m_iWeaponID].rage_recoil;
			g_Local.vNoRecoilAngle[1] = pparams->punchangle[1] * cvar.rage[g_Local.weapon.m_iWeaponID].rage_recoil;
